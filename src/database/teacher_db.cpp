...
std::vector<teacher::Teacher> TeacherDB::getAllTeachers() {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        teachers.emplace_back(sqlite3_column_int(stmt, 0), reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1)),
                              reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2)), patronymic,
                              reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4)),
                              reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5)));
    }
