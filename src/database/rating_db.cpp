...
bool RatingDB::hasExistingRating(int studentId, int entityId, bool isCourse) {
  ...
  if (isCourse) {
    sql =
        "SELECT 1 FROM course_ratings WHERE student_id = ? AND course_id = ?;";
  } else {
    sql =
...
RatingDB::Statistics RatingDB::getStatistics() {
  ...
  while (sqlite3_step(course_stmt) == SQLITE_ROW) {
    stats.courseStats.emplace_back(sqlite3_column_int(course_stmt, 0),
  ...
  while (sqlite3_step(teacher_stmt) == SQLITE_ROW) {
    stats.teacherStats.emplace_back(sqlite3_column_int(teacher_stmt, 0),
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ratings.emplace_back(sqlite3_column_int(stmt, 1),
                         ...
                         std::string(reinterpret_cast<const char*>(
                             sqlite3_column_text(stmt, 0))) == "course");
  }
...
std::vector<std::pair<int, int>> RatingDB::getCourseRatings(int studentId) {
  ...
  const char* sql =
      "SELECT course_id, rating FROM course_ratings WHERE student_id = ?;";
...
std::vector<std::pair<int, int>> RatingDB::getTeacherRatings(int studentId) {
  std::vector<std::pair<int, int>> ratings;
  const char* sql =
      "SELECT teacher_id, rating FROM teacher_ratings WHERE student_id = ?;";
