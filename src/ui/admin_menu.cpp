...
void AdminMenu::showStatistics() {
  ...
  for (const auto& [courseId, avg] : stats.courseStats) {
    try {
      ...
      std::cout << course.getName() << " (ID: " << courseId << "): " << avg
                << "/5\n";
    } catch (...) {
  ...
  for (const auto& [teacherId, avg] : stats.teacherStats) {
    try {
      ...
      std::cout << "Преподаватель ID:" << teacherId << " (удален): " << avg
                << "/5\n";
    }
  }
}
...
void AdminMenu::addTeacher() {
  ...
  try {
    database::TeacherDB teacherDB(db);
    int id = teacherDB.addTeacher(
        teacher::Teacher(0, surname, name, patronymic, institute, department));
